...
<head>
    ...
    <style>
        * {
        ...
        .summary div.total span:last-child {
            font-size: 18px;
            color: #4f46e5;
        }
...
<body>
    <div class="invoice">
        <div class="invoice-header">
            <div style="display: flex; align-items: center; gap: 16px;">
                @if(isset($settings['store_logo']) && $settings['store_logo'])
                    <img src="{{ Storage::url($settings['store_logo']) }}" alt="Logo"
                        style="height: 60px; width: auto; object-fit: contain;">
                @endif
                <div>
                    <div class="brand">{{ $settings['store_name'] ?? 'Kasir Modern' }}</div>
                    <div style="color:#64748b;font-size:12px;margin-top:4px;">{{ $settings['store_address'] ?? '' }}
                    </div>
        ...
        <div class="summary">
            ...
            <div><span>Diskon</span><span>Rp {{ number_format($transaction->discount_amount, 0, ',', '.') }}
                    ({{ $transaction->discount_percent }}%)</span></div>
            @if ($transaction->shipping_cost > 0)
                <div><span>Ongkir</span><span>Rp {{ number_format($transaction->shipping_cost, 0, ',', '.') }}</span></div>
            @endif
            <div class="total"><span>Total</span><span>Rp {{ number_format($transaction->total, 0, ',', '.') }}</span>
            </div>
        ...
        <a href="#" onclick="this.style.display='none'; window.print(); return false;" class="print-button no-print">🖨
            Cetak Invoice</a>
