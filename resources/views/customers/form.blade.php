...
@php
    ...
    $subtitle = $isEdit ? 'Perbarui data pelanggan' : 'Simpan data pelanggan untuk pelayanan lebih personal';
    $formAction = $isEdit ? route('customers.update', $customer) : route('customers.store');
    $isActive = old('is_active', $isEdit ? $customer->is_active : true);
@endphp
...
@section('content')
    <div class="max-w-3xl rounded-2xl bg-white p-6 shadow-sm border border-slate-200">
        <form action="{{ $formAction }}" method="POST" class="grid gap-6 md:grid-cols-2">
            @csrf
            @if ($isEdit)
                @method('PUT')
            @endif
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <textarea name="address" rows="3" class="mt-1 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200">{{ old('address', $isEdit ? $customer->address : '') }}</textarea>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input
                            type="text"
                            name="city"
                            value="{{ old('city', $isEdit ? $customer->city : '') }}"
                            class="mt-1 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                        >
                    </div>
                    <div>
                        ...
                        <input
                            type="text"
                            name="state"
                            value="{{ old('state', $isEdit ? $customer->state : '') }}"
                            class="mt-1 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                        >
                    </div>
                </div>
                <div>
                    ...
                    <input
                        type="text"
                        name="postal_code"
                        value="{{ old('postal_code', $isEdit ? $customer->postal_code : '') }}"
                        class="mt-1 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                    >
                </div>
            </div>
